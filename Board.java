...
public class Board {
    ...
    public void printBoard() {
        ...
        if (amIFirst == true) {
            System.out.print("\t\tPlayer vs. Opponent\tTotal Moves: " + totalTurns + "\n");
        } else {
            System.out.print("\t\tOpponent vs. Player\tTotal Moves: " + totalTurns + "\n");
        }
        ...
        for (int i = 0; i < gameBoard.length; i++) {
            ...
            // Must print the moves some how
               if (!moveList.isEmpty() && i == 0) {
                   if (moveList.size() <=1 ) {
                       System.out.print("\t\t" + moveList.get(0));
                   } else if (moveList.size() >= 2) {
                       System.out.print("\t\t" + moveList.get(0) + " " + moveList.get(1));
    ...
    */
    ...
    public int evaluation(char currentPlayer) {
        ...
        // Why does this happen when it isn't the case(?)
        /*
        if (horizontals == 4 || verticals == 4) {
    ...
    public int minimax(Board board, int depth, boolean isMaxPlayer, int alpha, int beta) {
        ...
        if (isMaxPlayer) {
            ...
            return minimax(successorList.get(bestBoardIndex), depth+1, false, 0 , 0);
            ...
            return minimax(successorList.get(bestBoardIndex), depth+1, true, 0 , 0);
        }
    }
    ...
    /*  The old DIY connect4 aiMove (Manually inputting your move)
...
*/
    ...
    public Board aiMove() {
        ...
        if (nextBoard.totalTurns == 0) {
            ...
            nextBoard.minimax(nextBoard, 1, true, 0 , 0);
            nextBoard.totalTurns++;
            
            
            //Used to print the possible successor boards for testing purposes
        ...
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = 0; j < gameBoard[i].length; j++) {
                if (gameBoard[i][j] != nextBoard.gameBoard[i][j]) {
                    nextBoard.move = String.valueOf((char) i + 65) + j;
                }
            }
        }
